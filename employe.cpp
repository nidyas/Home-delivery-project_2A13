...
//QDateTime datecreation = date.currentDateTime();
              //QString afficheDC = "Date de Creation PDF : " + datecreation.toString() ;
                     QPdfWriter pdf("C:\\Users\\hedia\\OneDrive\\Documents\\QTprinc\\pdf.pdf");
...
QString  EMPLOYE ::showold(QString name)
{
    QSqlQuery query2;
    query2.prepare("Select * from AJID where  ID= :id");
    query2.bindValue(":id", name);
    ...
    k += rec.value("salaire").toString();
    k += " NBR_HEURE: ";
    k += rec.value("NBR_HEURE").toString();
